        ...
        <form action="{{ route('backend.job_task.update', ['id' => $job->id, 'taskId' => $task->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="card-body">
                <h4 class="card-title mb-0">
                    Nhiệm vụ
                    <small class="text-muted">Sửa nhiệm vụ</small>
                </h4>
                <hr>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-10">
                        <input class="form-control" type="text" name="name" id="name" value="{{ $task->name ?? old('name') }}" placeholder="Nhập tên nhiệm vụ" required="" autofocus="">
                    </div><!--col-->
                </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-10">
                        <input class="form-control" type="date" name="deadline" value="{{ $task->deadline ?? old('deadline') }}" id="deadline" placeholder="Nhập ngày họp ..."  required="">
                    </div><!--col-->
                </div>
                    ...
                    <div class="col-md-10">
                        <select name="status_id" class="form-control" id="status_id">
                            @foreach($statuses as $status)
                                <option @if(!empty(old('status_id')) && old('status_id') === $status->id) selected @endif value="{{ $status->id }}">{{ $status->name }}</option>
                            @endforeach
                        </select>
                    </div><!--col-->
                </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-10">
                        <textarea class="form-control" name="description" id="description" placeholder="Nhập nội dung" rows="5">{{ $task->description ?? old('description') }}</textarea>
                    </div><!--col-->
                </div>
            ...
            <div class="card-footer">
                <div class="row">
                    ...
                    <div class="col text-right">
                        <button type="submit" class="btn btn-success btn-sm">@lang('labels.pages.backend.product.form.edit_submit')</button>
                    </div>
                </div>
            </div>
        </form>
